...
async function testPolicyLinter() {
    ...
    try {
        ...
        await dimrill.loadSchema(path.join(__dirname, "../tests/schemas/orders/products.dmrl.json"));
        ...
        assertMatch(
            invalidPolicyResult1.some(error => error.message.includes('Policy must have a Version')),
            true,
        ...
        assertMatch(
            invalidPolicyResult2.some(error => error.message.includes('Statement must have a valid Effect')),
            true,
        ...
        assertMatch(
            invalidPolicyResult3.some(error => error.message.includes('does not exist in schema')),
            true,
        ...
        assertMatch(
            invalidPolicyResult4.some(error => error.message.includes('Statement must have either Action or Ressource')),
            true,
        ...
        assertMatch(
            invalidPolicyResult5.some(error => error.message.includes('does not exist in the referenced schemas')),
            true,
        ...
        assertMatch(
            invalidPolicyResult6.some(error => error.message.includes('requires a number variable, but "orderCurrency" is of type "string"')),
            true,
        ...
        assertMatch(
            invalidPolicyResult7.some(error => error.message.includes('Unsupported condition operator')),
            true,
        ...
        assertMatch(validStatementResult, [], "Valid statement should produce no errors");
        ...
        const validConditionResult = dimrill.validateConditions(
            validCondition,
            {
                Effect: "Allow",
                Action: ["blackeye:orders:allowedProductCategories"]
        ...
        assertMatch(validConditionResult, [], "Valid condition should produce no errors");
