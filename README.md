  ...
  "orders": {
    "create": {
      ...
      "Condition": {
        ...
        "VariableEnforceTypeCast": { "userId": "ToObjectId" }
      }
    }
  }
}
...
- `VariableEnforceTypeCast`: Forces type casting for query values by variable name (e.g., `ToObjectId`). `QueryEnforceTypeCast` is accepted as a deprecated alias for one release; schema load/compile emits a `console.warn` telling you to migrate.
- `QueryKeys`: Optional explicit allow-list for MongoDB field paths generated by `ToQuery`. If omitted, non-dangerous field paths are allowed. Keys starting with `$` or prototype-polluting names (`__proto__`, `constructor`, `prototype`) are always denied.
    ...
    "Statement": [
      {
        "Effect": "Allow",
        "Action": ["orders:create"],
        "Condition": { "StringEquals:ToQuery": { "_id": "{{$userId}}" } }
      }
    ]
  }
]
...
// Result: { valid: true, query: { _id: ObjectId("507f1f77bcf86cd799439011") } }
```

`VariableEnforceTypeCast` casts the value resolved from `{{$userId}}`; it does not allow the query field. `QueryKeys` is the separate, optional allow-list for generated MongoDB field paths such as `_id` or `buyer.organization`.
