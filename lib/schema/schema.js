...
module.exports = class Schema {
  ...
  constructor(...args) {
    ...
    this.compile(args[0]); //make compilation mandatory
  }
  ...
  *compileExpression(expr) {}
   ...
   * @returns {array} - Returns an array of all routes.
   */
  createMap(Policies) {
    return Policies.reduce((arr, policy) => {
      if (
        policy.Pages &&
        Array.isArray(policy.Pages) &&
        policy.Pages.length >= 1
      ) {
        return [
  ...
  synthetize(drna, req) {
    ...
    */
    ...
    //querystring for fastify
    if (req.querystring && typeof req.querystring === "object") {
      formattedReq = { ...formattedReq, ...req.querystring };
    }
    const paramsMatched =
      Array.isArray(parameters) && parameters.length >= 1
        ? [...this.paramsMatcher(type, parameters, formattedReq)]
        : [];
    /*
...
*/
    ...
    let localPath = drna.replace(/\./g, "-").replace(/\s/g, "-").split(":");
