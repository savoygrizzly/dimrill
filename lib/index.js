...
function initialize(...args) {
  ...
  this.reqExtract =
    args[0].req && Array.isArray(args[0].req) ? args[0].req : [];
  return this;
}
 ...
 */
function reqExtractor(req) {
  const obj = {};

  if (Array.isArray(this.reqExtract) && this.reqExtract.length == 0) {
    return req;
  }
  this.reqExtract.forEach((key) => {
...
function authorize(drna, policies, req, user, context) {
  req = reqExtractor.call(this, req);
